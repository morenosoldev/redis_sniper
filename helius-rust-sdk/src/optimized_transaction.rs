...
impl Helius {
    ...
    pub async fn poll_transaction_confirmation(&self, txt_sig: Signature) -> Result<Signature> {
        ...
        loop {
            ...
            match
                self.connection().get_signature_status_with_commitment(&txt_sig, commitment_config)
            {
                Ok(Some(Ok(()))) => {
                    return Ok(txt_sig);
                }
                ...
                Err(err) => {
                    return Err(HeliusError::ClientError(err));
                }
            }
        }
    }
    ...
    pub async fn create_smart_transaction(
        ...
        if config.signers.is_empty() {
            return Err(
                HeliusError::InvalidInput("The fee payer must sign the transaction".to_string())
            );
        }

        let payer_pubkey: Pubkey = config.fee_payer.map_or(config.signers[0].pubkey(), |signer|
            signer.pubkey()
        );
        ...
        let existing_compute_budget_instructions: bool = config.instructions
            .iter()
            .any(|instruction| {
                instruction.program_id ==
                    ComputeBudgetInstruction::set_compute_unit_limit(0).program_id ||
                    instruction.program_id ==
                        ComputeBudgetInstruction::set_compute_unit_price(0).program_id
            });
        ...
        if is_versioned {
            let lookup_tables: &[AddressLookupTableAccount] = config.lookup_tables
                .as_deref()
                .unwrap_or_default();
            let v0_message: v0::Message = v0::Message::try_compile(
                &payer_pubkey,
        ...
        let priority_fee_estimate: GetPriorityFeeEstimateResponse = self
            .rpc()
            .get_priority_fee_estimate(priority_fee_request).await?;

        let priority_fee_recommendation: u64 = priority_fee_estimate.priority_fee_estimate.ok_or(
            HeliusError::InvalidInput("Priority fee estimate not available".to_string())
        )? as u64;

        // Add the compute unit price instruction with the estimated fee
        let compute_budget_ix: Instruction = ComputeBudgetInstruction::set_compute_unit_price(
        ...
        if is_versioned {
            ...
            Ok((SmartTransaction::Versioned(versioned_transaction.unwrap()), last_valid_block_hash))
        } else {
            let mut tx: Transaction = Transaction::new_with_payer(
                &final_instructions,
                Some(&payer_pubkey)
            ...
            Ok((SmartTransaction::Legacy(legacy_transaction.unwrap()), last_valid_block_hash))
        }
    }
    ...
    pub async fn send_smart_transaction(
        ...
        let (transaction, last_valid_block_height) = self.create_smart_transaction(
        ...
        while
            Instant::now().duration_since(start_time) < timeout ||
